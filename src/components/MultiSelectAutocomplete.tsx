...
type Props = {
  label: string;
  items: CatalogItem[];
  value: CatalogItem[]; // controlled
  onChange: (val: CatalogItem[]) => void;
  placeholder?: string;
  max?: number; // e.g., services 10, languages 5
  disabled?: boolean;
  required?: boolean;
  className?: string;
};
...
export default function MultiSelectAutocomplete({
  ...
  // EN-only search; collapse spaces; match name or aliases
  const filtered = useMemo(() => {
    const q = input.trim().toLowerCase();
    const base = items.filter(i => !selectedKeys.has(i.key));
    if (!q) return base;
    return base.filter(i => {
      const hay = (i.name + ' ' + (i.aliases?.join(' ') || '')).toLowerCase();
      return hay.includes(q);
    });
  }, [input, items, selectedKeys]);
  ...
  function addItem(item: CatalogItem) {
    ...
    onChange([...value, { key: item.key, name: item.name, emoji: item.emoji }]);
    setInput('');
    setOpen(false);
  }
    ...
    <div ref={ref} className={`w-full ${className || ''}`}>
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        {value.map(v => (
          <span
            key={v.key}
            className="inline-flex items-center gap-1 rounded-full border border-gray-300 px-2 py-1 text-xs"
          >
            <span>{v.emoji || '•'}</span>
            <span>{v.name}</span>
            <button
      ...
      <input
        ...
        onKeyDown={(e) => {
          if (e.key === 'Enter' && filtered[0]) { e.preventDefault(); addItem(filtered[0]); }
          if (e.key === 'Backspace' && input === '' && value.length > 0) {
            e.preventDefault();
            removeItem(value[value.length - 1].key);
          }
      ...
      {open && filtered.length > 0 && (
        <ul
          id="msa-listbox"
          role="listbox"
          className="mt-1 max-h-56 overflow-auto rounded-md border border-gray-200 bg-white shadow-lg"
        >
          {filtered.map(it => (
            <li
              key={it.key}
