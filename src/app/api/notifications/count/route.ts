...
export async function POST(req: NextRequest) {
 // inside the handler, avoid console.log on every hit in dev:
 const isDev = process.env.NODE_ENV !== 'production';
 // replace noisy logs like console.log('POST /api/notifications/count ...')
 if (!isDev) {
 ...
 adminDb.collection("bookings").where("masterId","==",userId).where("status","==","pending").get(),
 adminDb.collection("bookings").where("clientId","==",userId).where("status","==","pending").get(),
 ]);
 ...
 const chatsSnap = await adminDb.collection("chats").where("participants","array-contains",userId).limit(100).get();
 let unreadMessages = 0;
 for (const chatDoc of chatsSnap.docs) {
 const chatId = chatDoc.id;
 const readDoc = await adminDb.collection("chats").doc(chatId).collection("reads").doc(userId).get();
 const lastReadAt = readDoc.exists ? (readDoc.data()!.lastReadAt as any) : null;

 // Возьмём последние 50 сообщений и посчитаем те, что новее lastReadAt и не от самого пользователя
 let q = adminDb.collection("chats").doc(chatId).collection("messages").orderBy("createdAt","desc").limit(50);
 const msgsSnap = await q.get();
 for (const m of msgsSnap.docs) {
 const d:any = m.data();
 if (d.senderId === userId) continue;
 const ts = d.createdAt;
 const tMillis = ts?.toMillis ? ts.toMillis() : (ts?._seconds||0)*1000;
 const rMillis = lastReadAt?.toMillis ? lastReadAt.toMillis() : (lastReadAt?._seconds||0)*1000;
