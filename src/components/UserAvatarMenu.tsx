      ...
      .then((d) => {
        if (d?.ok)
          setDetails({
            ...
            bookingNotifications: d.bookingNotifications || 0,
          });
      })
      .catch(() => {});
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-44 rounded-lg border border-gray-200 bg-white shadow-lg">
          ...
          <Link
            ...
            className="block px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 flex items-center justify-between"
          >
            Dashboard{" "}
            {count ? (
