...
import { getAdminDb as adminDb } from '@/lib/firebaseAdmin';
...
export async function POST(req: NextRequest) {
 ...
 let query: FirebaseFirestore.Query = db.collection('masters');
 ...
 const db = adminDb();
 if (!db) throw new Error('Admin DB not initialized');
 const snap = await db.collection('masters').doc(body.cursor).get();
