...
function deriveCountry(formatted: string) {
  ...
  if (f.includes(', canada') || f.endsWith(' canada')) return { country: 'Canada', countryCode: 'CA' };
  if (f.includes(', united states') || f.includes(', usa') || f.endsWith(' usa')) return { country: 'United States', countryCode: 'US' };
...
const BodySchema = z.object({
  ...
  city: CityLoose,
  services: z.array(z.object({ key: z.string(), name: z.string(), emoji: z.string().optional() })).default([]),
  serviceKeys: z.array(z.string()).default([]),
  serviceNames: z.array(z.string()).default([]),
  languages: z.array(z.object({ key: z.string(), name: z.string(), emoji: z.string().optional() })).default([]),
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!json) return NextResponse.json({ ok: false, code: 'BAD_JSON', message: 'Invalid JSON body' }, { status: 400 });
    ...
    if (!d.cityKey) {
      return NextResponse.json({ ok: false, code: 'VALIDATION_ERROR', message: 'Missing cityKey' }, { status: 422 });
    }
    if (!d.masterName) {
      return NextResponse.json({ ok: false, code: 'VALIDATION_ERROR', message: 'Missing masterName' }, { status: 422 });
    }
    const formatted = d.city.formatted;
    const { country, countryCode } = {
      country: d.city.country,
      countryCode: d.city.countryCode,
      ...(!d.city.country || !d.city.countryCode ? deriveCountry(formatted) : {}),
    };
    ...
    const serviceKeys = (d.serviceKeys?.length ? d.serviceKeys : services.map(s => s.key)) ?? [];
    const serviceNames = (d.serviceNames?.length ? d.serviceNames : services.map(s => s.name)) ?? [];

    const languages = d.languages ?? [];
    const languageKeys = (d.languageKeys?.length ? d.languageKeys : languages.map(l => l.key)) ?? [];
    const languageNames = (d.languageNames?.length ? d.languageNames : languages.map(l => l.name)) ?? [];
    ...
    return NextResponse.json({ ok: true, id: cardId, threadKey, reviewId: reviewRef.id }, { status: 201 });
  } catch (e: any) {
    console.error('public-card POST failed', e);
    return NextResponse.json(
      { ok: false, code: 'INTERNAL', message: e?.message ?? 'Unexpected error' },
      { status: 500 }
    );
  }
}
