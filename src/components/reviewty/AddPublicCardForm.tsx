...
import { createPublicCardAndReview_BR } from '@/lib/createPublicReview';

// helper рядом с формой
const slugify = (s: string) => s.toLowerCase().normalize('NFKD').replace(/[^\w\s-]/g, '').trim().replace(/\s+/g, '-').replace(/-+/g, '-');
...
export function AddPublicCardForm({ 
  ...
  const isReady = !!masterName.trim() && !!city?.formatted && rating >= 1 && rating <= 5 && !!text.trim();
    ...
    try {
      ...
      if (files && files.length) {
        if (currentUser?.uid) {
          ...
          setError('You are not signed in. The review will be submitted without photos.');
        }
      }
      ...
      await createPublicCardAndReview_BR({
        mode: 'public-card',
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        />
        {city?.formatted ? (
          <p className="text-xs text-green-600 mt-1">Selected: {city.formatted}</p>
        ) : (
          <p className="text-xs text-amber-600 mt-1">Select a city from the list.</p>
        )}
      </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          <select
            onChange={(e) => {
              if (e.target.value) {
                const option = SERVICE_OPTIONS.find(opt => opt.value === e.target.value);
      ...
      <div>
        ...
        <div className="space-y-2">
          <select
            onChange={(e) => {
              if (e.target.value) {
                const option = LANGUAGE_OPTIONS.find(opt => opt.value === e.target.value);
          ...
          {languages.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {languages.map(language => (
                <span key={language.key} className="badge badge-secondary gap-1">
          ...
          {[1,2,3,4,5].map((v) => {
            ...
            return (
              ...
              >
                <svg
                  viewBox="0 0 20 20"
                  className={`h-5 w-5 ${active ? 'text-amber-500' : 'text-gray-300'} fill-current`}
                  aria-hidden="true"
                >
