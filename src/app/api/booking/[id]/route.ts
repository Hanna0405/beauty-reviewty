...
export async function PATCH(
  ...
  try {
    ...
    if (
      !status ||
      !["confirmed", "declined", "canceled", "completed"].includes(status)
    ) {
    ...
    if (shouldSendEmail) {
      try {
        ...
        const bookingData = updatedBooking.exists
          ? (updatedBooking.data() as any)
          : null;
        if (bookingData && bookingData.clientUid) {
          const clientUid =
            bookingData.clientUid || bookingData.clientId || null;
          if (clientUid) {
            ...
            if (!prefs.bookingStatusChangeEnabled) {
              console.log(
                "[email-booking] skip: client disabled booking status emails",
                {
              ...
              const clientProfileSnap = await db
                ...
                .doc(clientUid)
                .get();
              const clientProfile = clientProfileSnap.exists
                ? (clientProfileSnap.data() as any)
                : null;
              const bookingDateTimeText = formatWhenFromBooking(bookingData);
              const masterUid =
                bookingData.masterUid || bookingData.masterId || null;
              let masterName: string | null = null;
              if (masterUid) {
                try {
                  const masterProfileSnap = await db
                    .collection("profiles")
                    .doc(masterUid)
                    .get();
              ...
              if (bookingData.listingId) {
                try {
                  const ls = await db
                    .collection("listings")
                    .doc(bookingData.listingId)
                    .get();
                  if (ls.exists) {
                    const listingData = ls.data() as any;
                    serviceName =
                      listingData?.title || listingData?.serviceName || null;
                  }
                } catch {
              ...
              await sendBookingStatusEmailToClient({
                ...
                status:
                  status === "canceled"
                    ? "cancelled"
                    : (status as "confirmed" | "declined" | "cancelled"),
                dashboardBookingUrl,
              });
            }
          }
        ...
        console.error("[booking/[id]] email error", emailErr);
      }
    }
