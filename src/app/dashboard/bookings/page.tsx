...
import { clearClientBookingUnread } from "@/lib/notifications";
...
export default function BookingsPage() {
  ...
  // Clear client booking unread flags when page opens
  useEffect(() => {
    if (!user?.uid) return;
    clearClientBookingUnread(user.uid).catch((err) => {
      console.error(
  ...
  return (
    <div className="space-y-4 p-6">
      <Link href="/dashboard" className="inline-block">
        <h1 className="text-2xl font-bold hover:text-pink-600 transition">
          My bookings
        </h1>
      </Link>
              ...
              >
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-gray-500">
                    Date &amp; time:{" "}
                    {booking.startISO || booking.date || booking.startAt || "—"}
                  </p>
                </div>
